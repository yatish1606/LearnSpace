...
const Course1 = (props) => {
	...
	React.useEffect(() => {
		Axios.get(`https://dbms-back.herokuapp.com/quizfromcourse/${courseInfo._id}`)
		.then(res => {
			if(res.data.success) {
 ...
 const changeCourseNameStyles = {
	...
	const sendMessage = () => {
		...
		Axios.post(`https://dbms-back.herokuapp.com/message`, obj )
		.then(res => {
			if(res.data.success) {
				console.log(res.data.data)
			}
		})
		Axios.get(`https://dbms-back.herokuapp.com/messagesfromcourse/${loc[loc.length-1]}`)
		.then(res => {
			if(res.data.success) {
