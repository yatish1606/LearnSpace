...
const QuizQuestion = ({ history }) => {
  ...
  const addNewQuestionTextual = () => {
    ...
    let obj = {
      questionTitle: textualQuestionTitle,
      keywords,
      textualQuesMarks: textualQuesMarksInt,
      minChar: minCharInt,
      questionType: "text",
      QID: questionID,
      option1: null,
  ...
  const createQuiz = () => {
    ...
    questions
      ...
      .forEach((q) => textualMarks.push(q.textualQuesMarks));
    let totalMarks =
    ...
    Axios.post("http://dbms-back.herokuapp.com/quiz", quiz)
      .then((res) => {
        if (res.data.success) {
          ...
          if (insertID) {
            ...
            Promise.all(quiz.questions.map((ques) => {
      ...
      setTimeout(()=>{window.location.href = `/course/${parseInt(loc[loc.length - 1])}`},3000)
            ...
            {questions.map((q, index) => {
              return q.questionType === "mcq" ? (
                ...
                <RenderQuestionTextual
                  ...
                  QID={q.QID}
                  onDelete={onDeleteQuestion}
                  textualQuesMarks={q.textualQuesMarks}
                />
              );
            })}
          </React.Fragment>
      ...
      >
        ...
        <ul style={{ margin: 0, padding: 0, marginLeft: 20 }}>
          <li>
            ...
            >
              Once the quiz is created, it can be started at any time.
            </p>
          </li>
          <li>
            <p
            ...
            >
              After the quiz is started, you can close the quiz at any time.
              Post that, student responses will be collected.
            </p>
          </li>
        </ul>
